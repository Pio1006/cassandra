...
public class SSTableIndexWriter implements ColumnIndexWriter
{
    ...
    public SSTableIndexWriter(IndexDescriptor indexDescriptor, ColumnContext columnContext, NamedMemoryLimiter limiter,
    ...
    private void flushSegment() throws IOException
    {
        ...
        try
        {
            ...
            SegmentMetadata segmentMetadata = currentBuilder.flush(indexDescriptor, columnContext);
            ...
            if (segmentMetadata != null)
            {
                ...
                logger.debug(columnContext.logMessage("Flushed segment with {} cells for a total of {} in {} ms."),
            ...
            long globalBytesUsed = currentBuilder.release(columnContext);
            currentBuilder = null;
        ...
        {
            logger.error(columnContext.logMessage("Failed to build index for SSTable {}."), indexDescriptor.descriptor, t);
            indexDescriptor.deleteColumnIndex(columnContext.getIndexName());
    ...
    public void flush() throws IOException
    {
        ...
        try
        {
            ...
            indexDescriptor.createComponentOnDisk(IndexComponent.create(IndexComponent.Type.COLUMN_COMPLETION_MARKER, columnContext.getIndexName()));
        }
        finally
        {
            if (columnContext.getIndexMetrics() != null)
    ...
    public void abort(Throwable cause)
    {
        ...
        logger.warn(columnContext.logMessage("Aborting SSTable index flush for {}..."), indexDescriptor.descriptor, cause);
    ...
    private void compactSegments() throws IOException
    {
        ...
        try (SegmentMerger segmentMerger = SegmentMerger.newSegmentMerger(columnContext.isLiteral());
             PerIndexFiles perIndexFiles = new PerIndexFiles(indexDescriptor, columnContext, true))
        {
            ...
            segments.clear();
            segments.add(segmentMerger.merge(indexDescriptor, columnContext, minKey, maxKey, maxSSTableRowId));
        }
        finally
        {
            indexDescriptor.deleteTemporaryComponents(columnContext.getIndexName());
        }
    }
